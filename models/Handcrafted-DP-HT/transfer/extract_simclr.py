...
train_set, test_set = get_data("cifar10", augment=False)
trainloader = torch.utils.data.DataLoader(train_set, batch_size=len(train_set), shuffle=False, num_workers=2)
...
xtrain = xtrain.numpy().astype(np.float32).reshape(-1,32,32,3) / 255.0
xtest = xtest.numpy().astype(np.float32).reshape(-1,32,32,3) / 255.0
ytrain = ytrain[0:12400].numpy().reshape(-1)
...
np.save("transfer/features/simclr_r50_2x_sk1_train_lt.npy", features_train)
np.save("transfer/features/simclr_r50_2x_sk1_test_lt.npy", features_test)
