...
#def train_private(model, cls_num_list, per_cls_weights, train_loader, optimizer, hdp, clip, s_clip, noise_multiplier, n_acc_steps=1):
def train_private(model, train_loader, optimizer, hdp, clip, s_clip, noise_multiplier, n_acc_steps=1):
    ...
    #loss_f = LDAMLoss(cls_num_list = cls_num_list, weight=per_cls_weights) ###HT
...
def process_grad_batch_with_rp(params, clipping, s_clipping, sigma):
    ...
    for p in params:  #every layer
        if p.requires_grad == True:
            ...
            if idx_layer == 1:     
                weibull_rp = g_weibull.sample([pd,k])
                ...
                threshold_eig = sorted_eig[int(n/20)] #pd/10
